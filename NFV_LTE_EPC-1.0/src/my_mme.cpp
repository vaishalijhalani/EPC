...
//global variables (added by sagar)
int ran_listen_fd;
int slot_size;
...
void run(void *arg)
{
	...
	cout << targs->tid << endl;
	int core_id = targs->cid;
	int thread_id = targs->tid;
	...
	int udp_afour_port, udp_attach_port, udp_detach_port;


	queue<int> udp_free_ports;
	set<int> udp_use_ports;
	int udp_begin = udp_slot_begin + thread_id*slot_size;
	int udp_end = udp_begin + slot_size-1;

	for(int i = udp_begin; i <= udp_end; i++) {
		udp_free_ports.push(i);
	}
	...
	/*udp port setup start*/

	// //	udp attach 3 ports setup	---
	// int udp_attach_begin = udp_slot_one + thread_id*slot_size;
	// int udp_attach_end = udp_attach_begin + slot_size -1;
	// //int udp_attach_port;
	...
	// int udp_afour_begin = udp_slot_two + thread_id*slot_size;
	// int udp_afour_end = udp_afour_begin + slot_size -1;
	// //int udp_afour_port;
	...
	// int udp_detach_begin = udp_slot_three + thread_id*slot_size;
	// int udp_detach_end = udp_detach_begin + slot_size -1;
	// //int udp_detach_port;
	...
	//mux_init(mutex);
	pthread_mutex_lock(&mutex);   //added by sagar
	...
	pthread_mutex_unlock(&mutex);  //added by sagar
				...
				switch(act_type) {
					...
					case 1:
						...
						else
						{
							...
							if(pkt.s1ap_hdr.msg_type == 3)//Ran -- MMe --Sgw -- MMe -- Ran
							{
								...
								udp_attach_port = udp_free_ports.front();
								cout << "PORT attach: " << udp_attach_port << endl;
								udp_free_ports.pop();
								udp_use_ports.insert(udp_attach_port);						        
								...
								fddata.initial_fd = cur_fd;
								fddata.msui = mme_s1ap_ue_id;
								//added by sagar, for same port number
								fddata.port = udp_attach_port;
							...
							if(pkt.s1ap_hdr.msg_type == 4)
							{
								...
								*/
						        ...
						        //commented below line, same port change
						        //udp_afour_port = udp_free_ports.front();
						        //added below line for same port change
						        udp_afour_port = fddata.port;
						        cout << "PORTa4: " << udp_afour_port << endl; 
						        // udp_free_ports.pop();
						        // udp_use_ports.insert(udp_afour_port);
						        ...
						        //commented for port change
						        //udp_detach_port = udp_free_ports.front();
						        //added for port change
						        udp_detach_port = fddata.port;
						        cout << "PORT detach: " << udp_detach_port << endl;
						        //udp_free_ports.pop();
						        //udp_use_ports.insert(udp_detach_port);
					...
					case 4:
						...
						if((return_events[i].events & EPOLLIN))
						{
							...
							// udp_use_ports.erase(udp_attach_port);
							// udp_free_ports.push(udp_attach_port);
					...
					case 5:
							if((return_events[i].events & EPOLLIN))
							{
		    					...
		    					// udp_use_ports.erase(udp_afour_port);
		    					// udp_free_ports.push(udp_afour_port);
						...
						case 6:
							if((return_events[i].events & EPOLLIN))
							{
		    					...
		    					cout << "deleted " << udp_detach_port << endl;
		    					udp_use_ports.erase(udp_detach_port);
		    					udp_free_ports.push(udp_detach_port);
...
int main(int argc, char *argv[])
{
	...
	/* ran listen socket setup start (moved to main() by sagar)*/
	...
	if (setsockopt(ran_listen_fd, SOL_SOCKET, SO_REUSEADDR, &uflag, sizeof(uflag)) < 0)
    {
	...
	{
		cout<<"Error: RAN bind call"<<endl;
		exit(-1);
	}

	listen(ran_listen_fd, MAXCONN);

	/* ran listen socket setup end (moved to main by sagar)*/
   	...
   	// get udp slot size per thread  (slot_size global)
   	slot_size = udp_slot_size/num_t;
   	cout << slot_size << endl;
	...
	//thread data
    struct thread_args targs[num_t];
    ...
    for(int i = 0; i < num_t; i++) {
    	...
    	targs[i].tid = i;
    	targs[i].cid = i;
    	int rc = pthread_create(&threads[i], &attr, run, (void *)&targs[i]);
    	if(rc) {
    		cout << "Not able to create threads" << endl;
    		exit(-1);
    	}
